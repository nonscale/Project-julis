...
### 6.1. 2단계 스캔 동작 방식
-   **1차 스캔:** Upbit과 같은 거래소에서 제공하는 데이터 API를 통해 모든 종목의 *현재 시점 OHLCV, 거래대금, 시가총액 등 기본 데이터*를 한꺼번에 수신한다. 전략 빌더에서 구성된 수식 중 지표 및 `shift` 함수를 제외한 기본 조건(예: 거래대금, 시가총액 등)만을 사용하여 이 데이터를 기반으로 전체 시장 또는 선택된 관심종목을 대상으로 빠르게 종목을 필터링한다.
-   **2차 스캔:** 1차 스캔에서 필터링된 종목들을 대상으로, 각 종목별로 *최소 현재 기준 과거 50봉 이상의 OHLCV 데이터*를 개별적으로 수신한다. 이 데이터를 기반으로 전략 빌더에서 구성된 전체 수식(지표 및 `shift` 함수 포함)을 적용하여 정밀 분석을 수행한다.
...
1.  **전체 레이아웃:**
    ...
    -   배치 순서: 아래의 순서대로 페이지를 구성
        -   Strategy Management   Dashboard   Analysis (KRW-BTC)   Strategy Builder  로그아웃 순의 배치로 클릭시 페이지로 연결, 항상 보임
...
1.  **스캔 결과 연동:** 스캔 결과 테이블의 종목을 클릭하면 '개별 상세 분석 페이지'로 이동한다. (기준선 A 값 전달)
2.  **기준선 A (Baseline A) 정의:** 2차 스캔에서 특정 조건 충족 시, 그 시점의 값을 기준으로 '기준선 A'를 계산하는 로직을 추가한다.
3.  **개별 상세 분석 페이지:**
    -   **동적 실시간 차트:** 다양한 시간 주기의 실시간 차트를 조회하고, 차트 기반 매매 기능을 제공한다.
    -   **실시간 데이터 연동:** WebSocket을 통해 현재가, 호가창, 체결 내역 등을 실시간으로 표시한다.
    -   **수동 매매 인터페이스:** 시장가, 지정가 등 다양한 주문을 실행할 수 있는 UI를 제공한다.
    -   **진보된 자동 매매 기능:** '총 투자금', '초기 매수 비율' 등을 설정하여 '자동매매 시작' 시, '기준선 A' 기반의 초기 매수, 자동 물타기, 자동 익절/손절 로직을 실행한다.
